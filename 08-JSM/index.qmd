...
1.  Estimate 'visual' distance $D$ between
...
3.  Calibrate against visual and conventional test results <!-- XXX TODO: Illustrate with a picture XXX -->

## 📏 Measuring Distance

How to measure ["difference"]{.emph .red}/["distance"]{.emph .blue} between plots?

::: smaller
-   **Statistics**: KL-Divergence (actual vs. null) [when data generating process is known]{.emph .blue .small}
...
## 🏗️ Building a Model {visibility="hidden"}

-   Inputs:
    -   Image
    -   Number of observations
    -   Scagnostics computed from `cassowaryr` [@masonNumbatsCassowaryr2024]
        -   "monotonic", "sparse", "splines", "striped"
-   Output: $\widehat D$, compared to $D$ using MSE loss

## 🏗️ Building a Model {visibility="hidden"}

-   VGG16 backbone

-   Training Data: 80k images with $D$ uniformly distributed along 49 intervals between $[0, 7]$ and one with $D>7$.

-   Test Data: 8000 images allocated in the same way

-   Result: Model Violation Index $MVI := 10 + \widehat D - \log (n)$
...
```{r rd-human}
vss_32 <- readRDS("data/vss_32.rds")
...
experiment <- experiment %>%
  left_join(readRDS("data/actual_ss.rds"))
```
...
lineup_vss <- readRDS("data/lineup_vss.rds")
...
library(ggpcp)
pcp_dat <- mutate(experiment, type = ifelse(type == "polynomial", "Non-linearity", "Heteroskedasticity")) %>%
  mutate(type = factor(type, levels = c("Non-linearity", "Heteroskedasticity"))) %>%
...
hetero_dat <- filter(pcp_dat, type == "Heteroskedasticity")
nonlinear_dat <- filter(pcp_dat, type == "Non-linearity")
p1 <- hetero_dat %>%
  ...
  ggplot() +
  geom_pcp(aes_pcp(color = type), alpha = 0.5) +
  geom_pcp_boxes(aes_pcp(), fill = "transparent", color = "black") +
  geom_pcp_labels(aes_pcp()) +
  # geom_pcp_boxes(aes_pcp(), boxwidth = 0.1) +
  facet_wrap(~type) + 
  theme_bw() +
  scale_x_discrete(labels = c("Lineup", "AutoVI", "Conventional")) +
  scale_color_discrete(guide = 'none', drop = F) + 
  ylab("") +
  xlab("") +
...
p2 <- nonlinear_dat %>%
  ...
  geom_pcp(aes_pcp(color = type), alpha = 0.5) +
  geom_pcp_boxes(aes_pcp(), fill = "transparent", color = "black") +
  geom_pcp_labels(aes_pcp(), color = "black") +
  facet_wrap(~type) + 
  theme_bw() +
  scale_x_discrete(labels = c("Lineup", "AutoVI", "Conventional")) +
  scale_color_discrete(guide = 'none', drop = F) + 
...
::: notes
AutoVI is in between conventional and lineup tests in terms of conservatism. Lineups are still much more likely to not reject than AutoVI, but it is at least a step in the right direction.
:::
...
#| fig-width: 6
#| layout-ncol: 2
#| fig-cap: ['Normal view', 'Pass into `autovi`']
fitted_model <- lm(MEDV ~ RM + LSTAT + PTRATIO, data = housing)
...
my_vi$check_result <- readRDS("data/dino_check.rds")
my_vi$summary_plot() +
...
::: columns
::: {.column width="50%"}
![Weihao (Patrick) Li](Patrick.jpg){width="50%"}

[did all of the 🦾 work]{.emph .large .blue}
